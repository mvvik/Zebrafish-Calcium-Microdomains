...
recomputeFlag = 1;         % Recompute? (just plot if zero)
...
% --- Colors for special locations points (RGB triplets)
Clrs = [ 1,   0,   0;  ...  % red
         0,   0,   1;  ...  % blue
         1, 0.7,   0;  ...  % orange
         1,   0,   1;  ...  % magenta
...
% ---  Buffering capacities for two simulation runs:
ArrayKappa =  [ 100,  720 ];   
...
if recomputeFlag
    ...
    if isempty(gcp('nocreate')), parpool(2); end
    ...
    parfor n = 1 : 2
        ...
        cmd1 = sprintf('%s %s 1 %g', prog, Script, Kappa);
        [~, Msg1] = system(cmd1);
        fprintf('Pulse done: Kappa = %g\n', Kappa);
        if numel(Msg1) ~= 0, warning('Error: %s', Msg1); end

        % Compute [Ca2+] for the 190 ms post-pulse
        cmd2 = sprintf('%s %s 2 %g ', prog, Script, Kappa);
        [~, Msg2] = system(cmd2);
        fprintf('Post-pulse done: Kappa = %g\n', Kappa);
...
for n = 1 : 2
    ...
    for k = 1 : nLocs
        ...
        % --- Pulse trace
        fname1 = sprintf('DATA/Kappa%d_KD%d_iter1_Ca%d', Kappa, KD, k);
        [E1, XX, Y] = processAndPlot(fname1, 2, 0, 1, 2, Clrs(k,:));
        minY = Y(1); maxY = Y(end);

        % --- Post-pulse trace (concatenated with offset)
        fname2 = sprintf('DATA/Kappa%d_KD%d_iter2_Ca%d', Kappa, KD, k);
    ...
    fnameBin = sprintf('DATA/CaPulse%dms_Kappa%d_KD%d.dat', Pulse, Kappa, KD);
    f  = fopen(fnameBin, 'rb');
    if f < 0
        warning('Could not open %s', fnameBin);
    else
        hold on;
        titleStr = sprintf('Mobile buffer, \\kappa=%d, I_{Ca}=%.3gpA, Time=%dms', Kappa, ICa, Pulse);
